...
Before we do PCA we need to thnik or consider, wheter we should CALE our
input.
...
The 1st PC capture 44.27% of the original variance. Note that 72.6% in
the first 3 PCs.
...
![](class10_files/figure-gfm/unnamed-chunk-9-1.png)<!-- --> This is a
...
\#\#3. Hierarchical clustering First lets see if we can cluster the
original data Kmeans( x, centers = 2, nstart = 20) HCLUST( dist(x)) -\>
...
![](class10_files/figure-gfm/unnamed-chunk-12-1.png)<!-- --> This does
not look good\! Let’s try and combine the result of PCA with clustering
…

\#\#5. Combining Methods Let’s see if PCA improves or degrades the
...
To get out clusters out of this tree we neeed to CUT it with the
‘cuttree()’ function.
...
![](class10_files/figure-gfm/unnamed-chunk-19-1.png)<!-- --> \> Q17.
Which of these new patients should we prioritize for follow up based on
your results? = patient 2
